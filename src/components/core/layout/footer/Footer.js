                ...
                <ul className="space-y-2">
                  {services.items.map((item, index) => (
                    <li key={index} className="flex items-center text-sm">
                      <Image src={ICONS.serivces[item.name].path} alt={ICONS.serivces[item.name].alt} height={1000} width={1000} className='w-[21px]' />
                      <span className='ml-3 text-[#4D4D4D] font-[400] font-lora text-sm'>{item.title}</span>
                    </li>
                  ))}
                </ul>
              </FooterColumn>
              ...
              <FooterColumn title={quickLinks.title}>
                <ul className="space-y-3">
                  {quickLinks.links.map((link) => (
                    <li key={link.label}>
                      <Link href={link.href} className="text-sm font-[400] font-lora text-[#4D4D4D]">
                        {link.label}
                      </Link>
                    </li>
                  ))}
                ...
                <div className="border border-gray-200 rounded-md p-4">
                  <div className="flex justify-between items-center">
                    ...
                    <div className="flex flex-col items-end">
                      ...
                      <span className="text-teal-500 font-bold">{delivery.free}</span>
                    </div>
                  </div>
                  ...
                  <p className="mt-2 text-xs">
                    {delivery.info.split('click here').map((text, index, array) => {
                      if (index === array.length - 1) return text;
                      return (
                        <React.Fragment key={index}>
                          {text}
                          <a href="#" className="text-teal-500 hover:underline">click here</a>
                        </React.Fragment>
                      );
                    })}
                  </p>
